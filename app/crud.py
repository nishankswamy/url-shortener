...
def count_clicks(db: Session, link_id: int) -> int:
    return db.scalar(
        select(func.count(models.Click.id)).where(models.Click.link_id == link_id)
    ) or 0


def clicks_by_day(db: Session, link_id: int, days: int = 30) -> list[dict]:
    """Daily click counts, with zero-filled gaps so the chart doesn't lie."""
    since = datetime.now(timezone.utc) - timedelta(days=days - 1)
    rows = db.execute(
        select(models.Click.clicked_at).where(
...
def top_referrers(db: Session, link_id: int, limit: int = 10) -> list[dict]:
    stmt = (
